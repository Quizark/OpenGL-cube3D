...
bool miejsceDrabiny(float cubeX, float cubeY, float cubeZ) {
    if (cubeX == ledderX && ledderZ == 0.0f && cubeY >= ledderY)
        return true;
    else
        return false;
}
...
void drawLadder(float x, float z)
{
    ...
    // Ustawiamy kolor br¹zowy
    //glColor3f(0.5f, 0.25f, 0.0f);

    // Rysujemy pierwsz¹ pionow¹ liniê
    while (y2 < 5.0f) {
        // Rysujemy pierwsz¹ pionow¹ liniê
        glBegin(GL_LINES);
        glVertex3f(x + 0.25f, y1, z + 0.25f);
        glVertex3f(x + 0.25f, y2, z + 0.25f);
        glEnd();

        // Rysujemy drug¹ pionow¹ liniê
        glBegin(GL_LINES);
        glVertex3f(x - 0.25f, y1, z - 0.25f);
        glVertex3f(x - 0.25f, y2, z - 0.25f);
        glEnd();

        // Rysujemy liniê ³¹cz¹c¹ linie pionowe
        glBegin(GL_LINES);
        glVertex3f(x - 0.25f, y1, z - 0.25f);
        glVertex3f(x + 0.25f, y1, z - 0.25f);
        glEnd();

        // Zwiêkszamy koordynaty linii o 1.0f
        y1 += 1.0f;
        y2 += 1.0f;
    }
...
void renderScene() {
    ...
    if (ROZDZIAL == 1) {
        ...
        float movementSpeed2 = 0.1f;       
        float movementSpeed3 = 0.3f;       
        float movementSpeed4 = 0.5f;       
...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 't'://Dzia³anie drabiny
        if (ROZDZIAL == 5 && miejsceDrabiny(cubeX, cubeY, cubeZ))
            cubeY += 0.1f;
        break;
